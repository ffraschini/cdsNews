...
if (empty($_POST["contact-name"]) || empty($_POST["contact-email"]) || empty($_POST["contact-comments"])){
    ...
    if(!is_email($email)){
        ...
        if($validatedphone && $validatemail){
			...
			//$mailsender = new OutgoingEmailAgent("ffraschini@codigodelsur.com","ffraschini@codigodelsur.com", "Sulmora1020", "smtp.codigodelsur.com", 25, "0");
            $result = send_email("ffraschini@codigodelsur.com", $subject, $body);
			 if($result){
